...
no_match(){ ! grep -RInE "$1" "${@:2}" 2>/dev/null || { echo "FAIL: forbidden pattern: $1" >&2; exit 1; }; }
...
case "$GATE_ID" in
  ...
  02)
    ...
    test -n "$V"
    grep -RIn "$V" Cargo.toml release.toml README.md VERSION >/dev/null
    ;;

  03)
    say "Cargo metadata lock validation"
  ...
  12)
    say "Release verification script syntax"
  ...
  20)
    say "PWA/cache gate"
    must_file tools/check_pwa_cache.py
  ...
  21)
    ...
    grep -RIn "v7.3.0\\|7.3.0" EVIDENCE_REPORT.md SOURCE_MANIFEST.sha256 >/dev/null
    ;;

  22)
    say "Source manifest checksum format"
    must_file SOURCE_MANIFEST.sha256
  ...
  24)
    ...
    no_match "Neural Boundary Game v5\\.5\\.12|SOVEREIGN BOUNDARY SIMULATION · v3\\.0\\.0|Neural Boundary Game v3\\.0\\.0" README.md index.html .github/workflows scripts
    ;;

  25)
    say "Claim hygiene"
    no_match "clinical-grade|medical device|certified medical|real neural data ingestion|hardware stimulation control" README.md docs || true
    ;;
  ...
  26)
    ...
    grep -RIn "AGPL\\|Commercial" LICENSE COMMERCIAL_LICENSE.md Cargo.toml >/dev/null
    ;;

  27)
    say "Security surface"
    must_file SECURITY.md
    grep -RIn "security\\|vulnerability\\|contact\\|report" SECURITY.md >/dev/null
    ;;

  28)
    say "Privacy surface"
    must_file PRIVACY_NOTICE.md
    grep -RIn "privacy\\|telemetry\\|data" PRIVACY_NOTICE.md >/dev/null
    ;;

  29)
    say "Terms surface"
    must_file TERMS_OF_USE.md
    grep -RIn "terms\\|use\\|license" TERMS_OF_USE.md >/dev/null
    ;;

  30)
    say "No raw neural data warning"
    must_file docs/NO_RAW_NEURAL_DATA.md
    grep -RIn "No raw\\|no raw\\|simulation\\|simulated" docs/NO_RAW_NEURAL_DATA.md README.md >/dev/null
    ;;
  ...
  34)
    say "Generated artifacts are not committed"
    ! git ls-files | grep -E '(^|/)(target|dist|node_modules|release-assets)/' || {
      echo "FAIL: generated artifact committed"
      exit 1
    }
    ;;

  35)
    say "GitHub workflow surface"
