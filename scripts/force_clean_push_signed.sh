...
if [ "${I_UNDERSTAND_REWRITE_HISTORY:-}" != "YES" ]; then
  ...
  echo "Run only if you want a clean v1.3 history."
  echo
  echo "Use:"
...
if [ ! -f "$SIGN_KEY" ]; then
  echo "WARNING: signing key not found: $SIGN_KEY"
  echo "Commit may fail or be unverified."
fi
...
git add .
git commit -S -m "feat: release Neural Boundary Game v1.3.0 Foundation Grande Edition"
