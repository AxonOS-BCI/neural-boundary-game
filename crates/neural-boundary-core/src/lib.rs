...
//! (v7.3.0). Canonical source of truth (§16.2): the browser never computes
...
pub const CORE_VERSION: &str = "7.3.0";
...
/// (major<<16)|(minor<<8)|patch == 0x070300 == 459520 (§17.1).
pub const PRODUCT_VERSION_PACKED: u32 = (7 << 16) | (3 << 8);
...
mod lib_tests {
    ...
    fn version_constants_match_spec() {
        assert_eq!(PRODUCT_VERSION_PACKED, 0x070300);
        assert_eq!(PRODUCT_VERSION_PACKED, 459520);
        assert_eq!(ABI_VERSION, 3);
        assert_eq!(CORE_VERSION, "7.3.0");
