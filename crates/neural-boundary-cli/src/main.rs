...
const EXPECTED_SCHEMA: &str = "neural-boundary-replay-v1.4.0";

fn main() {
    let path = env::args().nth(1).unwrap_or_else(|| {
        eprintln!("usage: neural-boundary-cli vectors/replay-v1.4.0.json");
        process::exit(2);
    });
    ...
    let s = game.snapshot();
    println!("Replay OK");
    println!("Final trust: {}", s.trust);
    println!("Final risk: {}", s.risk);
    println!("Final integrity: {}", s.integrity);
    println!(
        "Boundary status: {}",
        if s.raw_leaks == 0 { "SEALED" } else { "BREACHED" }
    );
}
