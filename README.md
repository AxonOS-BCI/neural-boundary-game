...
![WASM](https://img.shields.io/badge/WASM-boundary%20core-20d6ff)
...
The project is built from a clean v1.3 baseline: no `web-sys`, no `wasm-bindgen`, no browser-binding dependency surface, no patched legacy glue.
...
This rewrites the remote `main` history. Use only when intentionally replacing the previous v1.0.x history with clean v1.3:
