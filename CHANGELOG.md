...
- Version bump to 7.9.812 across all canonical surfaces: core `CORE_VERSION`,
  packed product version `0x070900`, web display, service-worker cache, and
  storage namespace `nbg_v7_9_812_`.
- Repository consolidated to the single canonical Cognitive Sovereignty Console
  (`web/index.html`, flat WASM ABI v3); no standalone duplicate game entry.
- Re-recorded all 16 canonical replay vectors at `CORE_VERSION = 7.9.812`
  (action sequences unchanged; state hashes and the daily-seed cross-check
  updated for the new version string).

### Unchanged

- Game model, 7 actions, 8 metrics, 9 scenarios, grades, scoring, RNG
  (`xorshift64star-v1`) and state-hash algorithm (`fnv1a64-v1`) are identical to
  7.3.0 — no mechanics change. Replay schema `neural-boundary-replay-v3` and ABI v3 unchanged.
